...
<template>
 <nav id="nav" class="navbar bg-linear-to-r from-main-medium to-main-dark dark:from-main-dark dark:to-main-medium navbar-expand-lg w-100 p-2 px-4">
    ...
    <template v-else>
      <RouterLink class="navbar-brand" to="/"><img class="w-[50px]" alt='Translator' src="@/assets/img/tr.jpg"/></RouterLink>
    </template>
    <div class="ml-auto">
        <form class="form-inline d-flex mr-4" @submit="handleSubmit">
            ...
            name="api_select" class="bg-main-accent dark:placeholder:text-main-light border-0 text-theme-dark pl-2" v-model="store.query"/>
            <datalist id="apis">
            <option v-for="api in store.apis" :key="api.name" :value="api.name"></option>
            </datalist>
        </form>
    </div>
    <button @click="layout.toggleDarkMode" 
