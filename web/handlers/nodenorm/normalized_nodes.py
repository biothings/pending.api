...
async def get_normalized_nodes(
    ...
    nodes = await _lookup_curie_metadata(handler, curies, conflations)
    ...
    node_identifier_label_mapping = await _lookup_identifiers_with_labels(handler, nodes)
...
async def _lookup_curie_metadata(handler: BaseAPIHandler, curies: list[str], conflations: dict) -> list[NormalizedNode]:
    """
    ...
    curies, malformed_curies, identifier_result_lookup = await _lookup_equivalent_identifiers(
        handler, curies, curie_order
    )
    ...
    for input_curie in curies:
        ...
        if any(conflations.values()) and len(conflation_identifiers) > 0:
            ...
            conflation_curies, _, conflation_result_lookup = await _lookup_equivalent_identifiers(
                handler, conflation_identifiers, conflation_order
            )
...
async def _lookup_equivalent_identifiers(
    handler: BaseAPIHandler, curies: list[str], curie_order: dict
    ...
    index = handler.biothings.elasticsearch.metadata.indices["node"]
    term_search_result = await handler.biothings.elasticsearch.async_client.search(
...
async def _lookup_identifiers_with_labels(handler: BaseAPIHandler, nodes: list[NormalizedNode]) -> dict:
    """
    ...
    curie_order = {curie: index for index, curie in enumerate(curies)}
    _, _, curie_label_lookup = await handler._lookup_equivalent_identifiers(curies, curie_order)
