...
def subset_upload_worker(
    ...
    offset_end: int,
    collection_name: str,
    collection_connection_options: tuple[str],
    ...
    the nodenorm files
    """
    logger.info("Bulk Uploading to the DB %s (%s, %s)", input_file, offset_start, offset_end)
    ...
    upload_database = get_src_db(*collection_connection_options)
        ...
        while file_handle.tell() < offset_end:
            ...
            try:
                ...
                if len(buffer) >= buffer_size:
                    ...
                    _upload_buffer(collection, buffer)
                    buffer = []
                    canonical_identifiers = []
...
def _upload_buffer(collection: pymongo.collection.Collection, buffer: list[dict]):
    logger.debug("Inserting %s records ... ", len(buffer))
    try:
        t0 = time.perf_counter()
        document_group = [pymongo.InsertOne(d) for d in buffer]
        collection.bulk_write(document_group, ordered=False)
        logger.debug("Bulk Write Duration [%s]", time.perf_counter() - t0)
